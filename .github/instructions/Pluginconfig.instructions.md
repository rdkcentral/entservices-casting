...
- Each plugin must define <PluginName>.conf.in file that includes the following mandatory properties:
  ...
  - **callsign**: A unique identifier used to reference the plugin within the framework. Every callsign must use the prefix `org.rdk.` followed by the ENT Service name written in PascalCase (e.g., org.rdk.PersistentStore).
...
If the plugin's `mode` property is set to `"Off"`, the plugin is treated as in-process.

If the `mode` property is omitted or left empty, the plugin also defaults to in-process.

If the `mode` property is explicitly set to `"Local"`, the plugin runs out-of-process.

The `mode` property is configured in the plugin's CMakeLists.txt file.
